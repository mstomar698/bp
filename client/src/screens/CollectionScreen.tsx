...
const CollectionScreen = () => {
  ...
  const [collectionLength, setCollectionLength] = useState(collections[userEmail].length)
...
const currentItems = useMemo(() => {
  ...
  useEffect(() => {
    ...
    const userEmail: any = userInfo?.email
    const collectionLength = collections[userEmail] ? collections[userEmail].length : 0;
    setCollectionLength(collectionLength)
  }, [userInfo, collections, fetchCollectionsOnAdd, collectionLength]);
  return (
    <>
    {collectionLength >= 1 ? <><div className="text-green-300 min-h-screen flex flex-grow flex-col justify-start items-start">
  ...
  </div></> : <><div className="min-h-screen flex items-center justify-center text-green-300 flex-col"><h1>Collections is Empty Add Some books </h1> <Link to={'/'}>Return Home</Link></div></>  }
